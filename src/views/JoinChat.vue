        ...
        <div class="glass rounded-2xl p-8 max-w-md w-full animate-fade-in">
            ...
            <div v-else-if="!chatExists" class="text-center">
                <div class="text-red-400 mb-4">
                    ...
                    <p class="text-sm text-gray-400">
                        Возможно, чат был удален или ссылка неверна
                    </p>
                </div>
            ...
            <form v-else @submit.prevent="joinChat" class="space-y-6">
                ...
                <button
                    type="submit"
                    :disabled="
                        !userName.trim() || isJoining || participantCount >= 10
                    "
                    class="btn-primary w-full disabled:opacity-50 disabled:cursor-not-allowed"
                >
                    <span v-if="isJoining">Подключение...</span>
                    <span v-else-if="participantCount >= 10"
                        >Чат переполнен</span
                    >
...
async function joinChat() {
    ...
    try {
        ...
        if (success) {
            ...
            const currentCount = await chatStore.getChatParticipantCount(
                props.id
            )
            if (currentCount >= 10) {
